...
`)
	...
	if findings[0].Scope != "pipeline:traces" {
		t.Errorf("expected scope pipeline:traces, got %s", findings[0].Scope)
	}
}
...
`)
	...
	if findings[0].Scope != "pipeline:metrics" {
		t.Errorf("scope = %q, want pipeline:metrics", findings[0].Scope)
	}
}
...
func TestExporterInsecureTLS_IgnoresTap(t *testing.T) {
	cfg := mustParse(t, `
receivers:
  otlp:
exporters:
...
`)
	findings := (&ExporterInsecureTLS{}).Evaluate(cfg)
	if len(findings) != 0 {
		t.Errorf("expected 0 findings for tap exporter, got %d", len(findings))
	}
}
...
func TestExporterEndpointLocalhost_IgnoresTap(t *testing.T) {
	cfg := mustParse(t, `
receivers:
  otlp:
exporters:
...
func TestFilesystemScraperNoExclusions_Fires(t *testing.T) {
	cfg := mustParse(t, `
receivers:
  hostmetrics:
    collection_interval: 60s
...
service:
  pipelines:
    metrics:
      ...
      exporters: [debug]
`)
	findings := (&FilesystemScraperNoExclusions{}).Evaluate(cfg)
	if len(findings) != 1 {
		t.Fatalf("expected 1 finding, got %d", len(findings))
	}
	if findings[0].RuleID != "filesystem-scraper-no-exclusions" {
		t.Errorf("ruleId = %q", findings[0].RuleID)
	}
	if findings[0].Severity != rules.SeverityInfo {
...
func TestFilesystemScraperNoExclusions_PassesWithMountPointExclusions(t *testing.T) {
	cfg := mustParse(t, `
receivers:
  hostmetrics:
    collection_interval: 60s
...
`)
	findings := (&FilesystemScraperNoExclusions{}).Evaluate(cfg)
	if len(findings) != 0 {
		t.Errorf("expected 0 findings with exclude_mount_points, got %d", len(findings))
	}
}

func TestFilesystemScraperNoExclusions_PassesWithFsTypeExclusions(t *testing.T) {
	cfg := mustParse(t, `
receivers:
  hostmetrics:
    collection_interval: 60s
...
`)
	findings := (&FilesystemScraperNoExclusions{}).Evaluate(cfg)
	if len(findings) != 0 {
		t.Errorf("expected 0 findings with exclude_fs_types, got %d", len(findings))
	}
}

func TestFilesystemScraperNoExclusions_NoFire_NoFilesystemScraper(t *testing.T) {
	cfg := mustParse(t, `
receivers:
  hostmetrics:
    collection_interval: 60s
...
`)
	findings := (&FilesystemScraperNoExclusions{}).Evaluate(cfg)
	if len(findings) != 0 {
		t.Errorf("expected 0 findings when no filesystem scraper, got %d", len(findings))
	}
}

func TestFilesystemScraperNoExclusions_NoFire_NotHostmetrics(t *testing.T) {
	cfg := mustParse(t, `
receivers:
  otlp:
exporters:
...
`)
	findings := (&FilesystemScraperNoExclusions{}).Evaluate(cfg)
	if len(findings) != 0 {
		t.Errorf("expected 0 findings for non-hostmetrics receiver, got %d", len(findings))
	}
}

func TestFilesystemScraperNoExclusions_SlashedName(t *testing.T) {
	cfg := mustParse(t, `
receivers:
  hostmetrics/custom:
...
`)
	findings := (&FilesystemScraperNoExclusions{}).Evaluate(cfg)
	if len(findings) != 1 {
		t.Fatalf("expected 1 finding for slashed hostmetrics, got %d", len(findings))
	}
}

func TestFilesystemScraperNoExclusions_NoFire_NoScrapers(t *testing.T) {
	cfg := mustParse(t, `
receivers:
  hostmetrics:
    collection_interval: 60s
